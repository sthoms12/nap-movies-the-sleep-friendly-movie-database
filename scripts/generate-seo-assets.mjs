...
function movieArticle(movie, rank) {
  const meta = [`${movie.year}`, movie.duration ? `${movie.duration} min` : null, `Nap Index ${movie.napIndex}/10`]
    .filter(Boolean)
    .join(' · ');
  const tags = (movie.tags ?? []).map((tag) => escapeHtml(tag)).join(', ');
