...
export function AdminPage() {
  ...
  const resetMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-submissions'] });
      toast.success('Submission queue purged.');
    },
            ...
            <header className="mb-16 border-b border-retro-muted/30 pb-12">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
                <div>
                  ...
                  <p className="text-retro-text/60 text-xs mt-4 tracking-[0.2em] font-medium">Council Verification for Static Index Updates.</p>
                </div>
                <div className="flex gap-4 items-center">
                  <Button
                    onClick={() => {
                      if (window.confirm('PURGE PENDING SUBMISSIONS?')) resetMutation.mutate();
                    }}
              ...
              <div className="bg-retro-accent/5 border border-retro-accent/20 p-4 flex gap-4 items-center">
                ...
                <p className="text-[10px] tracking-widest font-bold uppercase text-retro-accent/80 leading-relaxed">
                  System Note: Approved proposals will be held in the moderation log until the next static build update cycle. The main Index is now immutable on the edge.
                </p>
              </div>
            </header>
            {isLoading ? (
